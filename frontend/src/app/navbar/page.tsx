...
export default function Navbar({ user, onLogout }: NavbarProps) {
  ...
  const handleLogout = () => {
    if (onLogout) {
      ...
      // Default logout behavior - removed localStorage
      router.push('/');
    }
  };
            ...
            {user ? (
              <div className="relative hidden md:block">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-purple-800 hover:bg-purple-700 rounded-full transition-all"
                >
                ...
                {/* User Dropdown Menu */}
                {isUserMenuOpen && (
                  <div className="absolute right-0 mt-2 w-56 bg-gradient-to-br from-purple-900 to-indigo-900 border border-purple-500/30 rounded-xl shadow-2xl overflow-hidden">
                    ...
                    <button
                      ...
                      className="w-full px-4 py-3 text-left text-white hover:bg-purple-800/50 transition-colors flex items-center gap-2"
                    >
                    ...
                    <div className="border-t border-purple-500/20">
                      <button
                        ...
                        className="w-full px-4 py-3 text-left text-red-400 hover:bg-purple-800/50 transition-colors flex items-center gap-2"
                      >
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden py-4 border-t border-purple-500/20">
            <div className="flex flex-col space-y-4">
              {user && (
                <div className="px-4 py-3 bg-purple-800/30 rounded-lg mb-2">
                  <p className="text-white font-semibold">{user?.name || 'User'}</p>
                  <p className="text-purple-300 text-sm">{user?.email || ''}</p>
                </div>
              )}
              ...
              <button 
                onClick={() => { router.push('/contracts'); setIsMobileMenuOpen(false); }}
                className="text-white hover:text-purple-300 transition-colors text-left"
              >
                Contracts
              </button>
              <button 
                onClick={() => { router.push('/avatar'); setIsMobileMenuOpen(false); }}
                className="text-white hover:text-purple-300 transition-colors text-left"
              >
                Avatar
              </button>
              <button 
                onClick={() => { router.push('/crypto'); setIsMobileMenuOpen(false); }}
                className="text-white hover:text-purple-300 transition-colors text-left"
              >
                Crypto Prices
              </button>
              <button 
                onClick={() => { router.push('/faq'); setIsMobileMenuOpen(false); }}
                className="text-white hover:text-purple-300 transition-colors text-left"
              >
                FAQ
              </button>
